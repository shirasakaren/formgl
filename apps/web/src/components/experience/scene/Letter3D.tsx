 ...
 */
export function Letter3D({ assets, style = 'fold', ribbonColor = '#8e1b1b' }: { assets: SceneAssets; style?: LetterStyle; ribbonColor?: string }) {
  ...
  const res = useMemo(() => {
    ...
    const ribbonMat = new THREE.MeshStandardMaterial({ color: ribbonColor, roughness: 0.45, metalness: 0.05, side: THREE.DoubleSide });
    ...
    return { front, back, bottom, top, topQuat, scroll, ribbonMat, band, loop, tail };
  ...
  useFrame((state) => {
    ...
    if (r > 0 && target.ready) {
      ...
      const closedOffset = res.scroll ? -(LH / 2) * (1 - us) : (LH / 4) * (1 - us);
  ...
  return (
    <group ref={group}>
      {res.scroll ? (
        <>
          <mesh geometry={res.scroll.geometry} material={res.front} castShadow receiveShadow />
          <mesh geometry={res.scroll.geometry} material={res.back} castShadow receiveShadow />
          <group ref={ribbon}>
            <mesh geometry={res.band} material={res.ribbonMat} castShadow />
            <group position={[0, 0, ROLL_R + 0.003]}>
              <mesh geometry={res.loop} material={res.ribbonMat} position={[0.0, 0.007, 0]} rotation={[0, 0, 0.5]} />
              <mesh geometry={res.loop} material={res.ribbonMat} position={[0.0, -0.007, 0]} rotation={[0, 0, -0.5]} />
              <mesh geometry={res.tail} material={res.ribbonMat} position={[0.003, 0, 0.001]} rotation={[0, 0.3, 2.6]} />
              <mesh geometry={res.tail} material={res.ribbonMat} position={[-0.003, 0, 0.001]} rotation={[0, -0.3, 3.6]} />
            </group>
          </group>
        </>
      ) : (
