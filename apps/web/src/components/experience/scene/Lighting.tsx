...
export function Lighting({ preset, assets, quality }: { preset: LightPreset; assets: SceneAssets; quality: Quality }) {
  ...
  const dapple = useMemo(
    () =>
      new DappleMap({
        size: quality === 'low' ? 512 : quality === 'medium' ? 768 : 1024,
        leafTexture: assets.leaves,
        open: preset.canopyOpen,
        count: quality === 'low' ? 2600 : quality === 'medium' ? 5200 : 8000,
      }),
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* compiled on first update instead */
    }
    return () => {
      scene.remove(target);
      dapple.dispose();
    };
  }, [dapple, scene, target, gl]);
  ...
  useFrame(({ gl, clock }, dt) => {
    // the canopy mask runs at ~24fps — plenty for slowly swaying leaves
    acc.current += dt;
    if (acc.current < (quality === 'low' ? 1 / 10 : 1 / 24)) return;
    acc.current = 0;
  ...
  return (
    <>
      ...
      <BakedEnvironment intensity={debugSun ? 0 : preset.envIntensity}>
